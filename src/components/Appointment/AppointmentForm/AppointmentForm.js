...
const AppointmentForm = ({ modalIsOpen, closeModal, appointmentName,date }) => {
    ...
    const onSubmit = data => {
       ...
       .then(success =>{
           if(success){
            closeModal();
            alert("Your appointment has been added successfully");
           }
       })
    }
    return (
        <div>
            ...
            >
                <h2 className="text-brand text-center"style={{color:"#1CC7C1"}}>{appointmentName}</h2>
                ...
                <form className="p-5" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="form-group mt-2">
                        <button type="submit" className="btn text-white w-25 float-end " style={{backgroundImage: "linear-gradient(90deg, #86ebe7, #1CC7C1)"}}>Send</button>
                    </div>
                </form>
            </Modal>
        </div>
